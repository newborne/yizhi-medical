package com.yizhi.hospital.service.impl;

import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.yizhi.hospital.mapper.HospitalSettingMapper;
import com.yizhi.hospital.mapper.ScheduleMapper;
import com.yizhi.hospital.model.HospitalSetting;
import com.yizhi.hospital.model.Schedule;
import com.yizhi.hospital.service.ApiService;
import com.yizhi.hospital.util.BeanUtils;
...
public class ApiServiceImpl implements ApiService {
    ...
    public String getHospitalCode() {
        ...
        return hospitalSetting.getHospitalCode();
    }
    ...
    private String getApiUrl() {
        HospitalSetting hospitalSetting = hospitalSettingMapper.selectById(1);
        return hospitalSetting.getApiUrl();
    }

    @Override
    public JSONObject getHospital() {
        Map<String, Object> paramMap = new HashMap<>();
        paramMap.put("hospitalCode", this.getHospitalCode());
        paramMap.put("timestamp", HttpRequestHelper.getTimestamp());
        paramMap.put("sign", MD5.encrypt(this.getSignKey()));
        JSONObject respone = HttpRequestHelper.sendRequest(paramMap, this.getApiUrl() + "/api/medical/manage/hospital/show");
    ...
    public boolean saveHospital(String data) {
        ...
        Map<String, Object> paramMap = new HashMap<>();
        paramMap.put("hospitalCode", this.getHospitalCode());
        paramMap.put("hospitalName", jsonObject.getString("hospitalName"));
        paramMap.put("hospitalType", jsonObject.getString("hospitalType"));
        ...
        paramMap.put("sign", MD5.encrypt(this.getSignKey()));
        JSONObject respone =
                HttpRequestHelper.sendRequest(paramMap, this.getApiUrl() + "/api/medical/manage/saveHospital");
        System.out.println(respone.toJSONString());
        if (null != respone && 200 == respone.getIntValue("code")) {
            return true;
        } else {
            throw new YizhiException(respone.getString("message"), 201);
        }
    }
    ...
    public Map<String, Object> findDepartment(int pageNum, int pageSize) {
        ...
        JSONObject respone = HttpRequestHelper.sendRequest(paramMap, this.getApiUrl() + "/api/medical/manage/department/list");
        if (null != respone && 200 == respone.getIntValue("code")) {
            JSONObject jsonObject = respone.getJSONObject("data");
    ...
    public boolean saveDepartment(String data) {
        ...
        for (int i = 0, len = jsonArray.size(); i < len; i++) {
            ...
            Map<String, Object> paramMap = new HashMap<>();
            paramMap.put("hospitalCode", this.getHospitalCode());
            paramMap.put("departCode", jsonObject.getString("departCode"));
            paramMap.put("departName", jsonObject.getString("departName"));
            paramMap.put("intro", jsonObject.getString("intro"));
            paramMap.put("bigDepartCode", jsonObject.getString("bigDepartCode"));
            paramMap.put("bigDepartName", jsonObject.getString("bigDepartName"));
            paramMap.put("timestamp", HttpRequestHelper.getTimestamp());
            paramMap.put("sign", MD5.encrypt(this.getSignKey()));
            JSONObject respone = HttpRequestHelper.sendRequest(paramMap, this.getApiUrl() + "/api/medical/manage/saveDepartment");
            System.out.println(respone.toJSONString());
            if (null == respone || 200 != respone.getIntValue("code")) {
                throw new YizhiException(respone.getString("message"), 201);
            }
    ...
    @Override
    public boolean removeDepartment(String departCode) {
        Map<String, Object> paramMap = new HashMap<>();
        paramMap.put("hospitalCode", this.getHospitalCode());
        paramMap.put("departCode", departCode);
        paramMap.put("timestamp", HttpRequestHelper.getTimestamp());
        paramMap.put("sign", HttpRequestHelper.getSign(paramMap, this.getSignKey()));
        JSONObject respone = HttpRequestHelper.sendRequest(paramMap, this.getApiUrl() + "/api/medical/manage/department/remove");
        System.out.println(respone.toJSONString());
        if (null != respone && 200 == respone.getIntValue("code")) {
            return true;
        } else {
    ...
    public Map<String, Object> findSchedule(int pageNum, int pageSize) {
        ...
        paramMap.put("timestamp", HttpRequestHelper.getTimestamp());
        paramMap.put("sign", HttpRequestHelper.getSign(paramMap, this.getSignKey()));
        JSONObject respone = HttpRequestHelper.sendRequest(paramMap, this.getApiUrl() + "/api/medical/manage/schedule/list");
        System.out.println(respone.toJSONString());
        if (null != respone && 200 == respone.getIntValue("code")) {
            JSONObject jsonObject = respone.getJSONObject("data");
            result.put("total", jsonObject.getLong("totalElements"));
    ...
    public boolean saveSchedule(String data) {
        ...
        for (int i = 0, len = jsonArray.size(); i < len; i++) {
            JSONObject jsonObject = jsonArray.getJSONObject(i);
            Long id = jsonObject.getLong("hospitalScheduleId");
            Schedule schedule = new Schedule();
            schedule.setId(id);
            schedule.setHospitalCode(this.getHospitalCode());
            schedule.setDepartCode(jsonObject.getString("departCode"));
            schedule.setTitle(jsonObject.getString("title"));
            schedule.setDoctorName(jsonObject.getString("doctorName"));
            ...
            Map<String, Object> paramMap = new HashMap<>();
            paramMap.put("hospitalCode", schedule.getHospitalCode());
            paramMap.put("departCode", schedule.getDepartCode());
            paramMap.put("title", schedule.getTitle());
            paramMap.put("doctorName", schedule.getDoctorName());
            ...
            paramMap.put("hospitalScheduleId", schedule.getId());
            paramMap.put("timestamp", HttpRequestHelper.getTimestamp());
            paramMap.put("sign", HttpRequestHelper.getSign(paramMap, this.getSignKey()));
            JSONObject respone = HttpRequestHelper.sendRequest(paramMap, this.getApiUrl() + "/api/medical/manage/saveSchedule");
            System.out.println(respone.toJSONString());
            if (null == respone || 200 != respone.getIntValue("code")) {
                throw new YizhiException(respone.getString("message"), 201);
            }
    ...
    @Override
    public boolean removeSchedule(String hospitalScheduleId) {
        Map<String, Object> paramMap = new HashMap<>();
        paramMap.put("hospitalCode", this.getHospitalCode());
        paramMap.put("hospitalScheduleId", hospitalScheduleId);
        paramMap.put("timestamp", HttpRequestHelper.getTimestamp());
        paramMap.put("sign", HttpRequestHelper.getSign(paramMap, this.getSignKey()));
        JSONObject respone = HttpRequestHelper.sendRequest(paramMap, this.getApiUrl() + "/api/medical/manage/schedule/remove");
        System.out.println(respone.toJSONString());
        if (null != respone && 200 == respone.getIntValue("code")) {
            return true;
        } else {
    ...
    public void saveBatchHospital() throws IOException {
        ...
        for (int i = 1, len = jsonArray.size(); i < len; i++) {
            JSONObject jsonObject = jsonArray.getJSONObject(i);
            Map<String, Object> paramMap = new HashMap<>();
            paramMap.put("hospitalCode", "1000_" + i);
            paramMap.put("hospitalName", jsonObject.getString("hospitalName"));
            paramMap.put("hospitalType", i % 3 == 0 ? 1 : i % 3 == 1 ? 2 : 4);
            ...
            JSONObject respone = HttpRequestHelper.sendRequest(paramMap, this.getApiUrl() + "/api/medical/manage/saveHospital");
            System.out.println(respone.toJSONString());
            if (null == respone || 200 != respone.getIntValue("code")) {
                throw new YizhiException(respone.getString("message"), 201);
            }
